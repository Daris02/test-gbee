...
```bash
test-gbee$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
test-gbee$ sam local start-api
test-gbee$ curl http://localhost:3000/
```
...
```bash
test-gbee$ sam logs -n HelloWorldFunction --stack-name test-gbee --tail
```
...
```bash
sam delete --stack-name test-gbee
```
